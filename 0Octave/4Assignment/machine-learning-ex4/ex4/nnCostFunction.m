...
for i = 1:m
	% forward prop got activations
	active_1 = [ 1 X(i, :)]';
	active_2 = [ 1 ; sigmoid(Theta1 * active_1)];
	active_3 = sigmoid(Theta2 * active_2);
	...
	% add cost to J
	J += sum(yi .* log(active_3) + (1 .- yi) .* log(1 .- active_3));
end

J *= (- 1 / m);
regular = sum(sum(Theta1(:, 2:end) .^ 2)) + sum(sum(Theta2(:, 2:end) .^ 2));
% regular = sum(sum(Theta1 .^ 2)) + sum(sum(Theta2 .^ 2));
regular *= (lambda / (2 * m));
J += regular;
Theta1_grad .*= (1 / m); 
Theta2_grad .*= (1 / m);
