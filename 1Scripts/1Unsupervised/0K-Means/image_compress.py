...
def reconstruct(mat, K):
	mat_centroids = k_means.find_centroids(mat, K, False)
...
def main():
	input_file = rd[1]
	try:
		compression = [int(_) for _ in rd[2:]]
	except:
	...
	output_file = input_file.rsplit(',')[0] + "_compressed_%s_bit" %\
	 str(compression) + Path(input_file).suffix
	compress(input_file, compression, output_file)


# for script access
if __name__ == '__main__':
	print("Image Compression (v1.5)")
	main()